...
import com.easemob.easeui.ui.EaseContactRemoveListener;
...
public class EaseChatFragment extends EaseBaseFragment implements
	...
	protected void setUpView() {
		...
		EMContactManager.getInstance()
				.addContactListener(contactRemoveListener);
	...
	/**
	 * by lzan13 设置阅后即焚模式的开关，在easeui中默认是关闭状态，需要在Demo层面调用此方法
	 * 
	 * @param isOpen
	 */
	public void setReadFire(boolean isOpen) {
		if (chatType == EaseConstant.CHATTYPE_SINGLE && isOpen) {
			...
			Toast.makeText(getActivity(), R.string.toast_read_fire_opened,
				...
				public void onClick(View v) {
					...
					if (isReadFire) {
						...
						Toast.makeText(getActivity(),
								R.string.toast_read_fire_close,
								Toast.LENGTH_LONG).show();
						isReadFire = false;
					} else {
						titleBar.setBackgroundResource(R.color.top_bar_red_bg);
						Toast.makeText(getActivity(),
								R.string.toast_read_fire_opened,
								Toast.LENGTH_LONG).show();
						isReadFire = true;
					}
				}
	...
	public void onEvent(EMNotifierEvent event) {
		...
		case EventNewMessage:
			...
			EMLog.d(TAG, "onEvent: From: " + message.getFrom() + " to: "
					+ message.getTo());
			// 如果是当前会话的消息，刷新聊天页面
		...
		case EventReadAck:
			...
			// file三种消息支持 )
			if (ackMessage.getBooleanAttribute(EaseConstant.EASE_ATTR_READFIRE,
					false)
					&& (ackMessage.getType() == Type.TXT
							|| ackMessage.getType() == Type.VOICE || ackMessage
							.getType() == Type.IMAGE)) {
